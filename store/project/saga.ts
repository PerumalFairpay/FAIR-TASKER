...
import {
    ...
    getProjectsSuccess, getProjectsFailure,
    getProjectSuccess, getProjectFailure,
    updateProjectSuccess, updateProjectFailure,
...
function* onCreateProject({ payload }: any): SagaIterator {
    try {
        ...
        if (response.data.success) {
            ...
            yield put(createProjectFailure(response.data.message || "Failed to create project"));
        }
    } catch (error: any) {
...
function* onGetProjects(): SagaIterator {
    try {
        ...
        if (response.data.success) {
            ...
            yield put(getProjectsFailure(response.data.message || "Failed to fetch projects"));
        }
    } catch (error: any) {
        yield put(getProjectsFailure(error.response?.data?.message || "Failed to fetch projects"));
    }
}
...
function* onGetProject({ payload }: any): SagaIterator {
    try {
        ...
        if (response.data.success) {
            ...
            yield put(getProjectFailure(response.data.message || "Failed to fetch project"));
        }
    } catch (error: any) {
        yield put(getProjectFailure(error.response?.data?.message || "Failed to fetch project"));
    }
}
...
function* onUpdateProject({ payload }: any): SagaIterator {
    try {
        ...
        if (response.data.success) {
            ...
            yield put(updateProjectFailure(response.data.message || "Failed to update project"));
        }
    } catch (error: any) {
...
function* onDeleteProject({ payload }: any): SagaIterator {
    try {
        ...
        if (response.data.success) {
            yield put(deleteProjectSuccess({ id: payload, message: response.data.message || "Project deleted successfully" }));
        } else {
            yield put(deleteProjectFailure(response.data.message || "Failed to delete project"));
        }
    } catch (error: any) {
